# Create a partially signed transaction from the details below

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

#!/bin/bash

# TXID of original transaction
TXID="23c19f37d4e92e9a115aab86e4edc1b92a51add4e0ed0034bb166314dde50e16"
VOUT0=0
VOUT1=1

# Recipient and amounts
DEST_ADDR="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
DEST_AMOUNT=0.20000000
FEE=0.00001000
TOTAL_INPUT=0.23679108
CHANGE_AMOUNT=$(printf "%.8f" $(echo "$TOTAL_INPUT - $DEST_AMOUNT - $FEE" | bc))

# Change address (from your wallet)
CHANGE_ADDR=$(bitcoin-cli -regtest getrawchangeaddress)

# echo "📤 Sending 0.2 BTC to: $DEST_ADDR"
# echo "🔁 Returning change of $CHANGE_AMOUNT BTC to: $CHANGE_ADDR"

# 1. Create the raw transaction (manual inputs/outputs)
RAW_TX=$(bitcoin-cli -regtest createrawtransaction \
"[{\"txid\":\"$TXID\",\"vout\":$VOUT0},{\"txid\":\"$TXID\",\"vout\":$VOUT1}]" \
"{\"$DEST_ADDR\":$DEST_AMOUNT, \"$CHANGE_ADDR\":$CHANGE_AMOUNT}")

# echo "🛠️ Raw transaction created."

# 2. Convert to PSBT
PSBT=$(bitcoin-cli -regtest converttopsbt "$RAW_TX")
# echo "✅ PSBT (partially signed) created:"
echo "$PSBT"
